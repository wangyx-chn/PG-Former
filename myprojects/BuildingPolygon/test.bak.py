...
class IoU(BaseMetric):
    ...
    def process(self, data_batch, data_samples):
        ...
        pred_poly = ((0.5+data_batch['pred_reg']).cpu().numpy() * np.array([ori_shape]).reshape(-1,1,2)).astype(np.int32)
        # pred_poly = pred_poly+input_poly
        # pred_cls = (data_batch['pred_cls'].squeeze()>0.5).cpu().numpy()
        real_gt = [((gt.cpu().numpy() * np.array([ori_shape[i]])).reshape(-1,1,2)).astype(np.int32) for i,gt in enumerate(data_samples['real_gt'])]

        # test_vis
        vis_dir = '/home/guning.wyx/code/mmengine/work_dirs/PolyRefineModel_v3_test2k_0.8disturb_swin_b_6decoder/test_vis'
        img_dir = '/home/guning.wyx/code/mmengine/data/WHUBuilding/polyrefine/equal_sample_dataset/image_patch'
        ...
        for b in range(pred_poly.shape[0]):
            ...
            img1 = np.copy(img)
            img2 = np.copy(img)
            img = cv2.polylines(img,[input_poly[b].reshape(-1,1,2)],color=(0,0,255),isClosed=True)
            img1 = cv2.polylines(img1,[pred_poly[b].reshape(-1,1,2)],color=(0,0,255),isClosed=True)
            img2 = cv2.polylines(img2,[real_gt[b].reshape(-1,1,2)],color=(0,0,255),isClosed=True)
            ...
            pred_mask = cv2.fillPoly(pred_mask,[pred_poly[b].reshape(-1,1,2)],color=1)
            ...
            self.results.append(dict(input_intersect=input_intersect,
                                     pred_intersect=pred_intersect,
    ...
    def compute_metrics(self, results):
        ...
        metrics = dict(input_iou=input_iou, pred_iou=pred_iou,ab_rate_before=ab_rate_before,total_input_i=total_input_i,
                       total_input_u=total_input_u,total_pred_i=total_pred_i,total_pred_u=total_pred_u)
        print(metrics)
        return metrics 

class GenIoU(BaseMetric):
    ...
    def process(self, data_batch, data_samples):
        ...
        # test_vis
        vis_dir = '/home/guning.wyx/code/mmengine/work_dirs/PolyGenDETR_AutoReg_sample50//test_val'
        img_dir = '/home/guning.wyx/code/mmengine/data/WHUBuilding/polyrefine/dataset_sample50_val/image_patch'
        # if osp.exists(vis_dir):
        #     shutil.rmtree(vis_dir)
        os.makedirs(vis_dir,exist_ok=True)
        ...
        for b in range(len(real_gt)):
            ...
            pred_coords = pred_poly[b][:pred_points_num]
            try:
            ...
            new_gt[:length] = gt
            self.results.append(dict(pred_intersect=pred_intersect,pred_union=pred_union,scores = pred_logits[b], gt=new_gt))
    ...
    def compute_metrics(self, results):
        ...
        metrics = dict(acc=np.sum((pred_scores>0.5)==gt_scores)/gt_scores.size, pred_iou=pred_iou)
        print(metrics)
        return metrics 
