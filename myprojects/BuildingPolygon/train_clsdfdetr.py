...
from models.dfdetr import DeformableDETR, NestedTensor
...
class PolyGenModel(BaseModel):
    ...
    def __init__(self,model_args):
        super().__init__()
        self.network = DeformableDETR(**model_args)
...
def main():
    ...
    train_dataloader = DataLoader(
        batch_size=1,
        dataset=train_set,
        batch_sampler=DynamicBatchSampler(dataset=train_set, sampler=DefaultSampler(train_set), max_token=6400, shuffle=True),
    ...
    runner = Runner(
        model=model,
        work_dir='./work_dirs/PolyGenDFDETR_CLS_sample50_layer2_full',
        load_from='/home/guning.wyx/code/mmengine/work_dirs/PolyGenDFDETR_CLS_sample50_layer2_full/epoch_48.pth',
        train_dataloader=train_dataloader,
        optim_wrapper=dict(
            type=OptimWrapper, optimizer=dict(lr=0.0001, betas=(0.9, 0.999), type='AdamW',eps=1e-8, weight_decay=0.01),
        ...
        train_cfg=dict(by_epoch=True, max_epochs=48, val_begin=60, val_interval=1),
...
if __name__ == '__main__':
    main()
    # model = DeformableDETR().cuda()
    # flops, params = profile(model, inputs=(NestedTensor(torch.randn(4, 3, 224, 224).cuda(),torch.zeros([4,224,224]).cuda()),))
