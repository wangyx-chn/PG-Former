...
from models.network import PolyRefinerV3
...
class PolyRefineModel(BaseModel):
    ...
    def __init__(self,model_args):
        super().__init__()
        self.network = PolyRefinerV3(**model_args)
        self.reg_loss = nn.MSELoss()
        self.cls_loss = nn.BCEWithLogitsLoss()
        self.bce_loss = nn.BCELoss()

    def forward(self, imgs, data_samples=None, mode='tensor'):
        input = data_samples['mask']['data']-0.5
        pad_mask = data_samples['mask']['pad_mask']
        # aug = random.uniform(0.5, 1.5)
        aug = 0.8
        data_samples['aug'] = aug
        # reg = self.network(imgs,input,pad_mask)
        reg = self.network(imgs,input*aug,pad_mask)
        weights = 1 - data_samples['mask']['pad_mask']
        target = data_samples['poly_target']['data'] -0.5
...
def main():
    ...
    runner = Runner(
        model=model,
        work_dir='./work_dirs/PolyRefineModel_v3_test2k_0.8disturb_swin_b_6decoder',
        # load_from='/home/guning.wyx/code/mmengine/work_dirs/PolyRefineModel_v3_test2k_0.8disturb_swin_b_6decoder/epoch_150.pth',
        ...
        train_cfg=dict(by_epoch=True, max_epochs=150, val_begin=30, val_interval=15),
