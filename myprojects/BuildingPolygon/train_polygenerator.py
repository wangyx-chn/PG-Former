...
from models.network import PolyGenerator
...
class PolyGenModel(BaseModel):
    ...
    def __init__(self,model_args):
        super().__init__()
        self.network = PolyGenerator(**model_args)
    ...
    def forward(self, imgs, data_samples=None, mode='tensor'):
        ...
        reg = self.network(imgs)
        # weights = 1 - data_samples['mask']['pad_mask']
        target = data_samples['poly_target']['data']
        reg_loss = self.reg_loss(reg, (target-0.5)*2) # 缩放到[-1,1]
...
def main():
    ...
    runner = Runner(
        model=model,
        work_dir='./work_dirs/PolyGeneratorModel_sample50',
        load_from='/home/guning.wyx/code/mmengine/work_dirs/PolyGeneratorModel_sample50/epoch_36.pth',
        ...
        param_scheduler = [dict(begin=0,by_epoch=False,end=500,start_factor=0.33,type='LinearLR'),
                            dict(begin=0,by_epoch=True,end=36,gamma=0.1,milestones=[20,],type='MultiStepLR'),],
        train_cfg=dict(by_epoch=True, max_epochs=36, val_begin=30, val_interval=3),
